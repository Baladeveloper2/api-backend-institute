...
export const getStudentExamReport = async (req, res) => {
  ...
  try {
    const report = await StudentExamReport.findOne({
      ...
      examId,
      status: 'completed',
    }).populate('answerDetails.questionId', 'questionTextEnglish questionTextTamil');
    ...
    return res.status(200).json({
      ...report.toObject(),
      examId: examData, // ✅ include full exam info manually
    });
  } catch (error) {
...
export const Questions =  async (req, res) => {
  try {
    ...
    const questionsForStudent = exam.ExamModel.map(q => ({
      questionNumber: q.questionNumber,
      questionText: q.questionText,
      options: q.options, 
    }));

    res.status(200).json({ examCode, ExamModel: questionsForStudent });
  } catch (error) {
    console.error(error);
